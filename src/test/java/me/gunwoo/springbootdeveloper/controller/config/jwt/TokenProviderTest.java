...
public class TokenProviderTest {
    ...
    void generateToken() {
        ...
        // given : 토큰에 유저 정보를 추가하기 위한 테스트 유저를 생성
        User testUser = userRepository.save(User.builder()
                .email("user@email.com")
                .password("test")
                .build());

        // when : 토큰 제공자의 generateToken() 메소드를 호출해서 토큰을 생성
        String token = tokenProvider.generateToken(testUser, Duration.ofDays(14));

        // then : jjwt 라이브러리를 사용해서 토큰을 복호화한 후, 토큰을 만들 때 클레임으로 넣어둔
        // id값이 given 절에서 만든 유저 ID와 동일한지 검증
